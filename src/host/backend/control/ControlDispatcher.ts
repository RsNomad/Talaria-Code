...
export class ControlDispatcher {
  ...
  // WS-GD.2a A6: `throwPollUnconfirmed` also has its own private copy on
  // `McpAdminHandler` now (the mcp-domain poll uses that one) — this
  // instance stays here, UNMOVED, because `handleSkillsAdmin`'s
  // `pollSkillInstall`/`pollSkillUninstall` (not yet moved — Task A7) still
  // call it; A7 gives skills its own private copy and removes this one.
  /**
