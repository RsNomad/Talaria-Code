...
describe('RootCoordinator (W4-T2 real per-root lease)', () => {
  ...
  it('A6: tracker is consulted at ACCESS time, not mint time', () => {
    const fakeTracker = {} as CheckpointTrackerLike;
    let current: CheckpointTrackerLike | undefined = undefined;
    const c = new RootCoordinator('/root', () => current);
    expect(c.tracker).toBeUndefined();
    current = fakeTracker;
    expect(c.tracker).toBe(fakeTracker);
    current = undefined;
    expect(c.tracker).toBeUndefined();
  });
