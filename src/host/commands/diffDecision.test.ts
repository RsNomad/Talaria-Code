...
describe('diffIdentityFromDiffTabInput — W4-T3b: pure (sessionId, toolId) extraction from a diff tab\'s URIs', () => {
  it('extracts the (sessionId, toolId) from the "modified" side', () => {
    expect(
      diffIdentityFromDiffTabInput({
        original: { scheme: 'talaria-diff', authority: 'before', path: '/session-1/tool-1/src/a.ts' },
        modified: { scheme: 'talaria-diff', authority: 'after', path: '/session-1/tool-1/src/a.ts' },
      }),
  ...
  it('returns undefined when neither side is a talaria-diff: URI (an ordinary file compare tab)', () => {
    expect(
      diffIdentityFromDiffTabInput({
  ...
  it('returns undefined when the URIs disagree on toolId (malformed/tampered — fail closed, not "pick one")', () => {
    expect(
      diffIdentityFromDiffTabInput({
        original: { scheme: 'talaria-diff', authority: 'before', path: '/session-1/tool-1/a.ts' },
        modified: { scheme: 'talaria-diff', authority: 'after', path: '/session-1/tool-2/a.ts' },
      }),
    ).toBeUndefined();
  });
  ...
  it('returns undefined when the URIs disagree on sessionId (fail closed)', () => {
    expect(
      diffIdentityFromDiffTabInput({
        original: { scheme: 'talaria-diff', authority: 'before', path: '/session-1/tool-1/a.ts' },
        modified: { scheme: 'talaria-diff', authority: 'after', path: '/session-2/tool-1/a.ts' },
      }),
    ).toBeUndefined();
  });
});
