/*
 * F1b (path doc §7.1): `must()` is the HOST test-file narrowing helper for
 * `noUncheckedIndexedAccess` fallout — a REAL runtime check (throws on
 * absence), not a `!` lie. Host version of `webview/src/testing/must.ts`
 * (F1a), same contract. This is its own characterization test: `must()`
