 ...
 * 300s default.
 */
describe('OpenAICompatFimBackend.streamFim — ADR-R2-06 stream deadlines (L2-CA-05)', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.useRealTimers();
  });
  ...
  it('reaps a mid-stream stall as StreamIdleTimeoutError once the 120s inter-chunk idle elapses', async () => {
    ...
    const backend = new OpenAICompatFimBackend({ apiBase: 'http://127.0.0.1:11434', model: 'qwen2.5-coder:1.5b-base' });
