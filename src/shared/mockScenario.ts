 ...
 * agent reasons, reads the file, proposes a 2-hunk patch, asks permission to run
 * `npm test`, updates a plan, and summarizes. `panelData` gives realistic
 * snapshots for all 8 control panels.
 *
...
/**
 * The approval id the MockBackend pauses on. It emits `mockTurn` up to and
 * including the `approval.request`, then waits for a `approval.respond` with
 * this id before continuing with the remaining steps.
 */
