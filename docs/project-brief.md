# Talaria Code — Project Brief (single source of truth)

Everyone building this reads this first. It pins the decisions so parallel work doesn't diverge.

## What we're building
Talaria Code: the in-editor client for **Hermes**, a full AI agent system. A VS Code webview panel: chat with the agent (streaming answer, reasoning, tool cards, file diffs with approval, plan) + side panels to manage the agent (Tools, MCP, Skills, Checkpoints, Subagents, Models, Settings).
...
```
Vscode-ext-new/
├── package.json, tsconfig*.json, esbuild.js, .vscodeignore, .gitignore, README.md   ← build/manifest layer
├── .vscode/launch.json, tasks.json                                                   ← build/manifest layer
├── src/
│   ├── extension.ts, host/**                                                          ← host/transport layer
│   └── shared/protocol.ts, shared/mockScenario.ts                                     ← shared contract
├── webview/  (vite root: index.html, vite.config.ts, src/**, tailwind config)         ← webview UI layer
└── docs/  (each layer writes its OWN doc: arch-host.md / arch-webview.md / build-setup.md / contract.md)
```
Rules to avoid conflicts:
- Only the build/manifest layer writes `package.json` and root build config.
- Only the shared-contract layer writes `src/shared/*`. The host and webview layers **import** `../shared/protocol` types by name; they do NOT define the protocol.
- The webview layer's Vite config outputs to `../dist/webview/`. The build/manifest layer's npm scripts orchestrate both builds.
- Each layer writes only inside its lane + its own `docs/*.md`.

## Definition of done for this pass (architecture + skeleton)
A coherent scaffold that (once the build/manifest layer's `npm install` + build run) F5-launches on Windows and shows the Talaria panel driven by mock data — chat streams a canned turn (reasoning → tool card → diff with accept/reject → approval → plan), and the side tabs switch to static-but-real-looking panels. No Hermes process involved.
